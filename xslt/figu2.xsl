...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <svg height="{$y-max + 100}" width="{$x-max+ 250}"
            ...
            <text x="{110+$x-max}" y="-{-160+$y-max}" text-alin="middle" >Character Counts</text>
            <rect x="{160+$x-max}" y="-{-200+$y-max}" height="50" fill="white" width="25" stroke="black" stroke-width="2"/>
            <text x="{140+$x-max}" y="-{-280+$y-max}" text-alin="middle" >Instances</text> 
            <rect x="{160+$x-max}" y="-{-320+$y-max}" height="50" fill="gray" width="25" stroke="black" stroke-width="2"/>
            <text x="{140+$x-max}" y="-{-400+$y-max}" text-alin="middle">Total Counts</text> 
            <!-- Legends -->
    ...
    <xsl:template match="chapter">
        <xsl:variable name="counts" select="count(.//*[@topic])"/>
        <xsl:variable name="weighted-counts" select="string-length(normalize-space(string-join(.//*[@topic], ' ')))"/>
